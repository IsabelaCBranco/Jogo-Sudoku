...
## lê o estado do tabuleiro e emite cliques para o GameController.
...
func _desenhar_destaques(origem: Vector2, lado: float) -> void:
	for l in SudokuBoard.TAMANHO:
		for c in SudokuBoard.TAMANHO:
			...
			if _selecionada == Vector2i(l, c):
				draw_rect(rect, COR_SELECIONADA)
			elif _celula_relacionada(Vector2i(l, c)):
...
func _lado_celula() -> float:
	return floor(min(size.x, size.y) / SudokuBoard.TAMANHO)
